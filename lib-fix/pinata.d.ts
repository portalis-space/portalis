...
declare class PinataClient {
  ...
  pinJobs(
    filters?: PinataPinJobsFilterOptions,
  ): Promise<import('./commands/pinning').PinataPinJobsResponse>;
  unpin(hashToUnpin: string): Promise<unknown>;
  pinList(
    filters: PinataPinListFilterOptions,
  ): Promise<import('./commands/data').PinataPinListResponse>;
  getFilesByCount(
    filters: PinataPinListFilterOptions,
    maxCount?: number,
  ): {
  ...
  testAuthentication(): Promise<
    import('./commands/data').PinataTestAuthenticationResponse
  >;
